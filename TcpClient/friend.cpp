...
void Friend::showAllOnllineUsr(PDU *pdu)
{
    if (NULL == pdu)
    {
        return;
...
void Friend::updateFriendList(PDU *pdu)
{
    ...
    for(uint i = 0; i < uiSize; i++)
    {
        ...
        //                             .arg(strcmp(caOnline, "1") == 0 ? "offline":"online"));
    }
}
...
void Friend::showOnline()
{
    if (m_pOnline->isHidden())
    {
        ...
        PDU *pdu = mkPDU(0);
        pdu->uiMsgType = ENUN_MSG_TYPE_ALL_ONLINE_REQUEST;
...
void Friend::searchUsr()
{
    ...
    if (!m_strSearchName.isEmpty())
    {
        ...
        PDU *pdu = mkPDU(0);
        memcpy(pdu->caData, m_strSearchName.toUtf8().toStdString().c_str(), m_strSearchName.size());
        pdu->uiMsgType = ENUN_MSG_TYPE_SEARCH_USR_REQUEST;
        TcpClient::getInstance().getTcpSocket().write((char*)pdu, pdu->uiPDULen);
        free(pdu);
        pdu = NULL;
    }
...
void Friend::delFriend()
{
    if(NULL != m_pFriendListWidget->currentItem())
    {
        ...
        memcpy(pdu->caData, strSelfName.toUtf8().toStdString().c_str(), strSelfName.size());
        memcpy(pdu->caData + 32, strFriendName.toUtf8().toStdString().c_str(), strFriendName.size());
        TcpClient::getInstance().getTcpSocket().write((char*)pdu, pdu->uiPDULen);
        free(pdu);
        pdu = NULL;
    }
...
void Friend::groupChat()
{
    ...
    if(strMsg == NULL)
    {
        QMessageBox::warning(this, "群发好友", "信息不能为空");
        return;
    }
    ...
    QString strName = TcpClient::getInstance().loginName();
    memcpy(pdu->caData, strName.toUtf8().toStdString().c_str(), strName.size());
    strncpy((char*)(pdu->caMsg), strMsg.toUtf8().toStdString().c_str(), strMsg.size());
    TcpClient::getInstance().getTcpSocket().write((char*)pdu, pdu->uiPDULen);
    free(pdu);
    pdu = NULL;
