...
void TcpClient::recvMsg()
{
    if (!OpeWidget::getInstance().getBook()->getDownloadStatus())
    {
        ...
        uint uiMsgLen = uiPDULen - sizeof(PDU); // 消息内容的长度
        ...
        switch (pdu->uiMsgType)
        {
            ...
            {
                OpeWidget::getInstance().getFriend()->showAllOnllineUsr(pdu);
                break;
            }
            ...
            case ENUM_MSG_TYPE_ADD_FRIEND_REQUEST:
            {
                ...
                PDU *respdu = mkPDU(0);
                memcpy(respdu->caData, pdu->caData, 32); //被加好友人姓名
                memcpy(respdu->caData + 32, pdu->caData + 32, 32); //加好友人名字
...
{
    QString strMsg = ui->lineEdit->text();  // 获得输入框信息
    if(!strMsg.isEmpty())
    {
        PDU *pdu = mkPDU(strMsg.size() + 1);
        pdu->uiMsgType = 8888;  // 消息类型（原：8888）
...
void TcpClient::on_login_pb_clicked()
{
    ...
    if(!strName.isEmpty() && !strPwd.isEmpty())
    {
        ...
        PDU *pdu = mkPDU(0);
        pdu-> uiMsgType = ENUN_MSG_TYPE_LOGIN_REQUEST;
        strncpy(pdu->caData, strName.toUtf8().toStdString().c_str(),32); //在caData中前32位放用户名
        strncpy(pdu->caData+32, strPwd.toStdString().c_str(),32); //caData的后32位放密码
        //把pdu发送出去
        m_tcpSocket.write((char*)pdu, pdu->uiPDULen);
        free(pdu);
        pdu = NULL;
    ...
    {
        QMessageBox::critical(this,"Login","Login failed: No UserName or Password");
    }
}
...
void TcpClient::on_regist_pb_clicked()
{
    ...
    {
        PDU *pdu = mkPDU(0);
        pdu-> uiMsgType = ENUN_MSG_TYPE_REGIST_REQUEST;
        strncpy(pdu->caData, strName.toUtf8().toStdString().c_str(),32); //在caData中前32位放用户名
        strncpy(pdu->caData+32, strPwd.toStdString().c_str(),32); //caData的后32位放密码
        //把pdu发送出去
        m_tcpSocket.write((char*)pdu, pdu->uiPDULen);
    ...
    {
        QMessageBox::critical(this,"Regist","Regist failed: No UserName or Password !!!");
    }
}
...
//显示密码
void TcpClient::on_disp_pb_toggled(bool checked)
{
      QLineEdit *pwdEdit = ui->pwd_le;
      QCheckBox *dispPb = ui->disp_pb;
