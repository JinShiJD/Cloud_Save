...
void PrivateChat::on_sendMsg_pb_clicked()
{
    ...
    if(!strMsg.isEmpty())
    {
        ...
        memcpy(pdu->caData, m_strLoginName.toUtf8().toStdString().c_str(), m_strLoginName.size());
        memcpy(pdu->caData+32, m_strChatName.toUtf8().toStdString().c_str(), m_strChatName.size());

        strcpy((char*)(pdu->caMsg), strMsg.toUtf8().toStdString().c_str());
