...
void MyTcpSocket::recvMsg()
{
    if(!m_bUpload)
    {
        ...
        switch (pdu->uiMsgType)
        {
            ...
            case ENUN_MSG_TYPE_LOGIN_REQUEST:
            {
                char caName[32] = {'\0'};
                char caPwd[32] = {'\0'};
                strncpy(caName, pdu->caData, 32);
            ...
            case ENUN_MSG_TYPE_ALL_ONLINE_REQUEST:
            {
            ...
            case ENUN_MSG_TYPE_SEARCH_USR_REQUEST:
            {
            ...
            case ENUM_MSG_TYPE_GROUP_CHAT_REQUEST:
            {
                ...
                qDebug() << "test here!!" <<onlineFriend;
                QString tmp;
                for(int i = 0; i < onlineFriend.size(); i ++)
                {
                    tmp = onlineFriend.at(i);
                    MyTcpServer::getInstance().resend(tmp.toUtf8().toStdString().c_str(), pdu);
                }
                break;
            }
    ...
    //                }
                    ...
                    memcpy(pFileInfo->caFileName, strFileName.toUtf8().toStdString().c_str(), strFileName.size());
    ...
    //                         <<" 常规文件:"<< fileInfoList[i].isFile();
            ...
            case ENUM_MSG_TYPE_DOWNLOAD_FILE_REQUEST:
            {
                ...
                m_file.open(QIODevice::ReadOnly);   //只读方式
                m_pTimer->start(1000);      // 开始计时

                break;
            }
            case ENUM_MSG_TYPE_SHARE_FILE_REQUEST:
            {
                qDebug() << "01接收文件的路径为： " << ((pdu->caMsg) + 32);
                ...
                memcpy(respdu->caMsg, (pdu->caMsg) + size, pdu->uiMsgLen - size); //文件路径

                qDebug() << "02接收文件的路径为： " << (respdu->caMsg);
            ...
            case ENUM_MSG_TYPE_SHARE_FILE_NOTE_RESPOND:
            {
                ...
                QString strShareFilePath = QString("%1").arg(pdu->caMsg);
            ...
            case ENUM_MSG_TYPE_MOVE_FILE_REQUEST:
            {
                ...
                qDebug() << "srcLen: " << srcLen << "destLen: " << destLen << "fileName: " << caFileName;
                ...
                memcpy(pDestPath, (pdu->caMsg) + srcLen, destLen);

                // 增加调试输出验证路径内容
                qDebug() << "源路径:" << pSrcPath;
                qDebug() << "目标基础路径:" << pDestPath;
