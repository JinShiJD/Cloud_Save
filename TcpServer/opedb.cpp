...
OpeDB::OpeDB(QObject *parent) : QObject(parent)
{
    ...
    if (!m_db.isValid()) {
        qDebug() << "Error: Unable to load the MySQL driver!";
    } else {
...
void OpeDB::init()
{
    ...
    m_db.setDatabaseName("D:\\MySQL8.0.30\\MySQL Server 8.0\\Data\\cloud.db");
...
void OpeDB::init()
{
    ...
    m_db.setDatabaseName("cloud"); //数据库名
...
//    qDebug() << "Available drivers: " << QSqlDatabase::drivers();
    ...
    {
        QMessageBox::critical(NULL, "打开数据库", "打开数据库失败：" + m_db.lastError().text());
    }
...
void OpeDB::handleAgreeAddFriend(const char *pername, const char *name)
{
    ...
    QString data = QString("insert into friend(id, friendId) "
                           "values((select id from usrInfo where name = \'%1\'), "
                           "(select id from usrInfo where name = \'%2\'))").arg(pername).arg(name);
    qDebug() << "add Friends : " << data << "!! " ;
    QSqlQuery query;
    query.exec(data);
}
